...
import { Eye, Users, Trophy, ExternalLink, Music } from 'lucide-react'
    ...
    const fetchTopDisplays = async () => {
      try {
        ...
        if (response.ok) {
          ...
          console.error('Failed to fetch top displays:', response.status)
        }
      } catch (error) {
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8 text-center">
          <div className="flex items-center justify-center space-x-2 mb-4">
            ...
            <h1 className="text-4xl font-bold text-foreground">JamLog Leaderboard</h1>
            <Trophy className="h-8 w-8 text-yellow-500" />
          </div>
                  ...
                  >
                    <div className="flex items-center space-x-4">
                      ...
                      <div className="flex-1">
                        <h3 className="font-semibold text-lg">@{display.identifier}</h3>
                        {display.lastTrack ? (
                          <div className="flex items-center space-x-1 text-sm text-muted-foreground">
                            <Music className="h-3 w-3" />
                            <span>{display.lastTrack.name} by {display.lastTrack.artist}</span>
                          </div>
                        ) : (
                    ...
                    <div className="flex items-center space-x-4">
                      ...
                      <div className="text-right">
                        <div className="flex items-center space-x-1 text-sm">
                          ...
                          <span className="font-medium">{display.viewCount?.toLocaleString() || '0'}</span>
                        </div>
